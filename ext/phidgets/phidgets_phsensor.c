...
int ph_phsensor_on_ph_change(PhidgetPHSensorHandle phid, void *userPtr, double ph);
...
VALUE ph_phsensor_get_ph(VALUE self) {
  PhidgetPHSensorHandle handle = (PhidgetPHSensorHandle)get_ph_handle(self);
  double ph;
  ph_raise(PhidgetPHSensor_getPH(handle, &ph));
...
VALUE ph_phsensor_get_ph_min(VALUE self) {
  PhidgetPHSensorHandle handle = (PhidgetPHSensorHandle)get_ph_handle(self);
  double ph;
  ph_raise(PhidgetPHSensor_getPHMin(handle, &ph));
  return rb_float_new(ph);
}
...
VALUE ph_phsensor_get_ph_max(VALUE self) {
  PhidgetPHSensorHandle handle = (PhidgetPHSensorHandle)get_ph_handle(self);
  double ph;
  ph_raise(PhidgetPHSensor_getPHMax(handle, &ph));
  return rb_float_new(ph);
}
...
VALUE ph_phsensor_get_ph_change_trigger(VALUE self) {
  PhidgetPHSensorHandle handle = (PhidgetPHSensorHandle)get_ph_handle(self);
  double trigger;
  ph_raise(PhidgetPHSensor_getPHChangeTrigger(handle, &trigger));
  return rb_float_new(trigger);
}

VALUE ph_phsensor_set_ph_change_trigger(VALUE self, VALUE trigger) {
  PhidgetPHSensorHandle handle = (PhidgetPHSensorHandle)get_ph_handle(self);
  ph_raise(PhidgetPHSensor_setPHChangeTrigger(handle, NUM2DBL(trigger)));
  return Qnil;
}
...
VALUE ph_phsensor_get_potential(VALUE self) {
  PhidgetPHSensorHandle handle = (PhidgetPHSensorHandle)get_ph_handle(self);
  double potential;
  ph_raise(PhidgetPHSensor_getPotential(handle, &potential));
...
VALUE ph_phsensor_get_potential_min(VALUE self) {
  PhidgetPHSensorHandle handle = (PhidgetPHSensorHandle)get_ph_handle(self);
  double potential;
  ph_raise(PhidgetPHSensor_getPotentialMin(handle, &potential));
  return rb_float_new(potential);
}
...
VALUE ph_phsensor_get_potential_max(VALUE self) {
  PhidgetPHSensorHandle handle = (PhidgetPHSensorHandle)get_ph_handle(self);
  double potential;
  ph_raise(PhidgetPHSensor_getPotentialMax(handle, &potential));
  return rb_float_new(potential);
}

VALUE ph_phsensor_set_temperature(VALUE self, VALUE temperature) {
  PhidgetPHSensorHandle handle = (PhidgetPHSensorHandle)get_ph_handle(self);
  ph_raise(PhidgetPHSensor_setTemperature(handle, NUM2DBL(temperature)));
  return Qnil;
}
...
VALUE ph_phsensor_set_on_ph_change_handler(VALUE self, VALUE handler) {
  ...
  if( TYPE(handler) == T_NIL ) {
    ...
    ph_raise(PhidgetPHSensor_set_OnPHChange_Handler((PhidgetPHSensorHandle)ph->handle, NULL, (void *)NULL));
  } else {
    callback_data->called = false;
    callback_data->exit = false;
    callback_data->phidget = self;
    callback_data->callback = handler;
    ph_raise(PhidgetPHSensor_set_OnPHChange_Handler((PhidgetPHSensorHandle)ph->handle, ph_phsensor_on_ph_change, (void *)callback_data));
